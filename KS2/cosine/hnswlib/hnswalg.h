...
namespace hnswlib {
    ...
    public:
        ...
        HierarchicalNSW(size_t m, float diff, int level, int vecdim0, int LSH_level, int LSH_vecdim0, SpaceInterface<dist_t> *s, SpaceInterface<dist_t> *s1, SpaceInterface<dist_t> *s2, SpaceInterface<dist_t> *s3, size_t max_elements, size_t M = 16, size_t ef_construction = 200, size_t random_seed = 100) :
            ...
            len_ =  16 * (2 * sizeof(unsigned short int));
			len1_ =  16 * ( sizeof(unsigned short int) );

            level_generator_.seed(random_seed);
            update_probability_generator_.seed(random_seed + 1);

            size_links_level0_ = maxM0_ * (2 * sizeof(unsigned short int) + sizeof(tableint) + LSH_level_) + sizeof(linklistsizeint);   //new
            ...
            size_links_per_element_ = maxM_ * (2 * sizeof(unsigned short int) + sizeof(tableint) + LSH_level_) + sizeof(linklistsizeint);
            mult_ = 1 / log(1.0 * M_);
            revSize_ = 1.0 / mult_;
        }
        ...
        inline short int* getLastNorm(int* data, int i) const {
			...
			int b = i % 16; 			
			char* data2 = (char*) data;
			data2 += maxM0_ * (sizeof(tableint) + 2 * sizeof(unsigned short int)) + a * len1_ + b * sizeof(unsigned short int);
			return (short int* )data2;
        }
        ...
        inline short int* getLastNorm16(int* data, int i) const {
			...
			int b = i % 16; 			
			char* data2 = (char*) data;
			data2 += 16 * (sizeof(tableint) + 2 * sizeof(unsigned short int)) + a * len1_ + b * sizeof(unsigned short int);
			return (short int* )data2;
        }
        ...
        inline short int* getLastNorm32(int* data, int i) const {
			...
			int b = i % 16; 			
			char* data2 = (char*) data;
			data2 += maxM_ * (sizeof(tableint) + 2 * sizeof(unsigned short int)) + a * len1_ + b * sizeof(unsigned short int);
			return (short int* )data2;
        }		
        ...
        inline short int* getLastNorm48(int* data, int i) const {
			...
			int b = i % 16; 			
			char* data2 = (char*) data;
			data2 += 48 * (sizeof(tableint) + 2 * sizeof(unsigned short int)) + a * len1_ + b * sizeof(unsigned short int);
			return (short int* )data2;
        }	

        inline unsigned char* getLSHM(int* data, int level, int i) const {
			char* data2 = (char*) data;
			data2 += maxM0_ * (sizeof(int) + 2 * sizeof(unsigned short int) + level) + i;
			return (unsigned char* )data2;
        }

        inline unsigned char* getLSHM16(int* data, int level, int i) const {
			char* data2 = (char*) data;
			data2 += 16 * (sizeof(int) + 2 * sizeof(unsigned short int) + level) + i;
			return (unsigned char* )data2;
        }

        inline unsigned char* getLSHM32(int* data, int level, int i) const {
			char* data2 = (char*) data;
			data2 += maxM_ * (sizeof(int) + 2 * sizeof(unsigned short int) + level) + i;
			return (unsigned char* )data2;
        }		

        inline unsigned char* getLSHM48(int* data, int level, int i) const {
			char* data2 = (char*) data;
			data2 += 48 * (sizeof(int) + 2 * sizeof(unsigned short int) + level) + i;
			return (unsigned char* )data2;
        }

        inline char* getDataByInternalIdQuery(tableint internal_id) const {
...
//                    data = (int *) (linkLists_[curNodeNum] + (layer - 1) * size_links_per_element_);
        ...
        void searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef, unsigned int* result, int K, float*** lsh_vec, float** query_lsh) const {
			...
			float ip0 = fstipfunc_(data_point, getDataByInternalIdQuery(ep_id), dist_func_param_);
			float dist0 =  -1 * ip0;
            ...
            while (k < LL) {
                ...
                if (retset[k].flag) {
					...
					if(l_num < LL){	
                        ...
                        for (size_t j = 1; j <= size; j++) {					
                            ...
                            if (!(visited_array[candidate_id] == visited_array_tag)) {
			                    ...
			                    float ip_ = fstipfunc_(data_point, getDataByInternalIdQuery(candidate_id), dist_func_param_);
			                    float dist = -1 * ip_;
						...
						float val = retset[LL-1].distance / diffadj_;  
                    ...
                    if(round == 1){
						...
						sum1 = _mm512_div_ps(temp_sub, v_float);						
						...
						v_int = _mm512_cvtepu16_epi32(v_short);
						v_float = _mm512_cvtepi32_ps(v_int);
						sum2 = _mm512_div_ps(temp_sub, v_float);	
						...
						v_int = _mm512_cvtepu16_epi32(v_short);
						v_float = _mm512_cvtepi32_ps(v_int);
						sum3 = _mm512_div_ps(temp_sub, v_float);	
						...
						v_int = _mm512_cvtepu16_epi32(v_short);
						v_float = _mm512_cvtepi32_ps(v_int);
						sum4 = _mm512_div_ps(temp_sub, v_float);	
...
/*				
				if(level == 0){
				int vv = sz_link_list_other + 1;
				printf("2---size = %d, cur_c = %d\n", vv, cur_c);
				}
*/				
				//-------test-------------------------------
                    } else {
...
*/				
        ...
        void saveIndex(const std::string &location, float diff3) {
            ...
            //size_t new_size = size_data_per_element16_ * num16 + size_data_per_element32_ * num32 + size_data_per_element_ * num64;
        ...
        void loadIndex(const std::string &location, SpaceInterface<dist_t> *s, SpaceInterface<dist_t> *s1, SpaceInterface<dist_t> *s2, SpaceInterface<dist_t> *s3, float diff3, int LSH_level, int LSH_vecdim0, size_t max_elements_i=0) {
		   ...
		   //---------------------------------
            ...
            size_links_per_element_ = maxM_ * (2 * sizeof(unsigned short int) + sizeof(tableint) + LSH_level) + sizeof(linklistsizeint);

		    //size_links_level0_ = maxM0_ * (2 * sizeof(short int) + sizeof(tableint) + level_ + LSH_level_ + sizeof(float)) + sizeof(linklistsizeint);
...
/*
				...
				for(int j = 0; j < vecdim_; j++){
					Norm += LSH_data[j] * LSH_data[j];
				}
				Norm = sqrt(Norm);

				for(int j = 0; j < vecdim_; j++){
					LSH_data[j] = LSH_data[j] / Norm;
				}	
				
				error_val = error_val / Norm;
*/
...
//---------------------------------------------------------
        ...
        void searchKnn(float *query_data, size_t k, unsigned int* result, float*** lsh_vec, float** query_lsh) const {
			...
			dist_t curdist = -1 * fstipfunc_(query_data, getDataByInternalIdQuery(enterpoint_node_), dist_func_param_);
            ...
            for (int level = maxlevel_; level > 0; level--) {
                ...
                while (changed) {
                    ...
                    for (int i = 0; i < size; i++) {
			            ...
			            dist_t d = -1 * fstipfunc_(query_data, getDataByInternalIdQuery(cand), dist_func_param_);
        ...
        void compression(int vecsize, int vecdim, bool* is_zero){
            ...
            sec_part_ = data_size_ + sizeof(labeltype);  
            size_links_level16_ = 16 * (2 * sizeof(unsigned short int) + sizeof(tableint) +LSH_level_ ) + sizeof(linklistsizeint);   //new
            size_data_per_element16_ = size_links_level16_ + data_size_ + sizeof(labeltype); 
			
            size_links_level32_ = 32 * (2 * sizeof(unsigned short int) + sizeof(tableint) + LSH_level_) + sizeof(linklistsizeint);   //new
            size_data_per_element32_ = size_links_level32_ + data_size_ + sizeof(labeltype); 

            size_links_level48_ = 48 * (2 * sizeof(unsigned short int) + sizeof(tableint) + LSH_level_) + sizeof(linklistsizeint);   //new
        ...
        void Boost(int i, Elem* nearest_neighbor, Elem* sorted_neighbor, int Msize) {
            BoostBaseLayer(i, getDataByInternalId(i), eff, nearest_neighbor, sorted_neighbor, &Msize);
			return;
        };

        void MRNG(int cur_id, Elem* cur_nearest_neighbor, Elem* cur_sorted_neighbor, Elem** nearest_neighbor, Elem** sorted_neighbor, int* rst, int* Msize, int* rst_size) {
			
			int pointer = 0;
			for(int i = 0; i < Msize[i]; i++){
				int obj_id = cur_nearest_neighbor[i].id;
				float dist1 = cur_nearest_neighbor[i].dist;
				
				Elem* obj_sorted_neighbor = sorted_neighbor[obj_id];
                int index1 = 0;
                int index2 = 0;
                bool flag = false;				
				while(true){
					if(obj_sorted_neighbor[index1].id < cur_sorted_neighbor[index2].id){
						index1++;
					}
					else if(obj_sorted_neighbor[index1].id > cur_sorted_neighbor[index2].id){
						index2++;
					}
					else{
						float dist2 = obj_sorted_neighbor[index1].dist;
						float dist3 = cur_sorted_neighbor[index2].dist;
						if(dist2 < dist1 || dist3 < dist1){  //in the lune
                            flag = true; 
							break;
						}
						index1++;
						...
						if(index1 >= Msize[obj_id] || index2 >= Msize[cur_id])
							break;
					}						
				}
				if(flag == false){
        ...
        void AddEdge(int i, int* rst, int rst_size) {
			
			tableint ii = i;
            int *data = (int *) get_linklist0(ii);
			size_t size = getListCount((linklistsizeint*)data);
			if (size >= maxM0_) return;
			
			int *datal = data + 1;
			
			size_t new_size = size;
			for(int j = 0; j < rst_size; j++){
				int obj_id = rst[i];
				bool flag = false;
				for(int i = 0; i < size; i++){
					if(obj_id == datal[i]){
				...
				if(flag == false){
					data[new_size] = obj_id;
					new_size++;
					if (new_size >= maxM0_) break;
				}	
			}
							
            setListCount((linklistsizeint*)data, (size_t) new_size);			
			return;
        };

        void BoostBaseLayer(tableint ep_id, const void *data_point, size_t ef, Elem* nearest_neighbor, Elem* sorted_neighbor, int* Msize) const {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            float dist0 = fstdistfunc_(data_point, getDataByInternalId(ep_id), dist_func_param_);
            dist0 = sqrt(dist0);
            float ip0 = dist0;			
			...
			for(int i = 0; i < cur_size; i++){
				if(retset[i].id == ep_id){ //skip the same node
					for(int j = i; j < cur_size - 1; j++){
						retset[j].id == retset[j+1].id;
					}
					cur_size -= 1;
					break;
				} 			
			...
			*Msize = cur_size;
			...
			for(int i = 0; i < cur_size; i++){
				nearest_neighbor[i].id = retset[i].id;
				sorted_neighbor[i].id = retset[i].id;
				nearest_neighbor[i].dist = retset[i].distance;
				sorted_neighbor[i].dist = retset[i].distance;
			}
			
			qsort(sorted_neighbor, cur_size, sizeof(Elem), cmp_int);
